...
parameters {
  ...
  real<lower=0> phi_prov; // overdispersion 
  ...
  array[nD] real<lower=0> phi; // overdispersion
  ...
  real<lower=0> kappa_phi; // standard deviation for district level sampling of phi
...
model {
  ...
  for(d in 1:nD) {
    ...
    phi[d] ~ normal(phi_prov, kappa_phi);
  }
  
  //--- likelihood 
  for(d in 1:nD) for(t in 1:nT) {
    cases[d][t,] ~ neg_binomial_2(Ecases[d][t,], phi[d]); // negative binomial likelihood
  }
...
generated quantities {
  ...
  for(d in 1:nD) {
    for(t in 1:nT) {
      for(a in 1:nA) {
        log_lik[d,t,a] = neg_binomial_2_lpmf(cases[d,t,a] | Ecases[d,t,a],phi[d]);
        y_rep[d,t,a] = neg_binomial_2_rng(Ecases[d,t,a],phi[d]);
