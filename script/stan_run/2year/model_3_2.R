...
if (model_no %in% c(1:2)){
  data_list <- readRDS("data/data_2year_list.rds") 
} else if (model_no %in% c(3:4)){
  data_list <- readRDS("data/data_indiv_2year_list.rds") 
}
...
# save output
saveRDS(stanfit,paste0("output/stan_fit/fit_",model_no,"_",province,"_2year.rds"))